...
//병드민 : 접수ON, 예약OFF, 진료항목ON(영유아검진),  사전문진OFF
...
//예약OFF
'STEP - 예약 OFF'
if(WebUI.waitForElementVisible(findTestObject('병드민설정_데일리/btn_resrvSetting'), GlobalVariable.waitTime))
{
	'STEP - [예약 설정] 버튼 선택'
	WebUI.click(findTestObject('병드민설정_데일리/btn_resrvSetting'), FailureHandling.CONTINUE_ON_FAILURE)
	
	'STEP - "예약 사용 중지" 위치로 이동'
	WebUI.scrollToElement(findTestObject('병드민설정_데일리/btn_reserv_unsubscribe'), GlobalVariable.waitTime, FailureHandling.CONTINUE_ON_FAILURE)

	'STEP - [똑닥 예약 사용중지] 버튼 선택'
	WebUI.click(findTestObject('병드민설정_데일리/btn_reserv_unsubscribe'), FailureHandling.CONTINUE_ON_FAILURE)
	
	WebUI.delay(2)
	
	'STEP - [사용 중지] 버튼 선택'
	WebUI.click(findTestObject('병드민설정_데일리/btn_off'), FailureHandling.CONTINUE_ON_FAILURE)
}
