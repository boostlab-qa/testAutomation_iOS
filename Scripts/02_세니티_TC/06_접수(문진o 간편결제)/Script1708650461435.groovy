...
//STEP01 - 병드민 사전문진 On 설정 후 토스페이 접수 신청
'STEP - 병드민 설정 테스트케이스 불러오기'
Mobile.callTestCase(findTestCase('00_병드민설정_세니티/01_병드민_세니티_접수(문진o)'), [:])
...
'STEP - 로그아웃'
if (Mobile.waitForElementPresent(findTestObject('테스트/05_마이페이지/btn_optionalBtn'), GlobalVariable.waitTime))
{
		'STEP - 마이페이지 [수정] 버튼 선택'
...
Mobile.setText(findTestObject('테스트/02_로그인/input_etPassword'), 'qwer1234!', GlobalVariable.fixedTime)
...
'STEP - 진료대상 본인 선택'
Mobile.tap(findTestObject('테스트/09_접수/txt_name', [('text') : '박명근']), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)
...
'STEP - 하단 결제수단 영역이 노출된면 결제수단 선택하여 접수 신청완료 화면으로 이동'
...
{
	'STEP - 하단 결제수단 영역이 노출되지 않으면 [다음] 버튼 선택'
	Mobile.tap(findTestObject('테스트/09_접수/btn_next'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)
...
'기대결과 - 사전문진표 화면이 노출된다.'
Mobile.verifyElementVisible(findTestObject('테스트/09_접수/txt_tv_questionnaire'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)

'STEP - 사전문진표 화면에서 [x] 버튼 선택'
Mobile.tap(findTestObject('테스트/09_접수/btn_question_close'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)

Mobile.delay(2)
	
'STEP - [종료하기] 버튼 선택'
Mobile.tap(findTestObject('테스트/09_접수/btn_question_cancel'), GlobalVariable.fixedTime, FailureHandling.CONTINUE_ON_FAILURE)
...
'기대결과 - 결제정보 영역에 간편결제 정보가 노출되지 않는다.'
Mobile.verifyElementNotVisible(findTestObject('테스트/04_접수내역/txt_methodValue', [('text') : '자동결제(토스페이)']), GlobalVariable.waitTime, FailureHandling.CONTINUE_ON_FAILURE)
